...
pub async fn add_work_schedule(props: WorkRecord) -> Result<bool, String> {
    ...
    conn.execute(
        "INSERT INTO work_schedule (date, start_time, end_time, hourly_wage, minutes, amount, note) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        rusqlite::params![props.date, props.start_time, props.end_time, props.hourly_wage, props.minutes, props.amount, props.note],
    )

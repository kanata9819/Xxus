...
fn transcoded_reader(path: &str) -> Result<Box<dyn Read + Send>, String> {
    ...
    let mut sniff_reader = BufReader::new(file);
    ...
    // 本読み込み用に再オープン
    let file = File::open(path).map_err(|e| format!("reopen error: {e}"))?;
...
pub async fn import_csv(path: String) -> Result<bool, String> {
    ...
    let transcoded = transcoded_reader(&path)?;
